...
export const verificarReferencia= asyncHandler(async (req: Request, res: Response) => { res.json(await svc.verificarReferencia(String(req.params.token))) })
export const verificarReferenciaEmpresa = asyncHandler(async (req: Request, res: Response) => { res.json(await svc.verificarReferenciaEmpresa(Number(req.params.id), Number(req.params.rid))) })
