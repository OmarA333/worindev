...
import { Star, Zap, CheckCircle2, ArrowRight, TrendingUp } from 'lucide-react';

interface Props { onNavigate: (path: string) => void; }

const DIMENSIONES = [
  { key: 'hard',  label: 'Habilidades Técnicas',        pct: 40, score: 85, icon: '🎓', color: 'from-primary-500 to-primary-700',  desc: 'Títulos, certificaciones y experiencia técnica' },
  { key: 'soft',  label: 'Habilidades Blandas y Psico', pct: 20, score: 72, icon: '🧠', color: 'from-accent-500 to-accent-700',    desc: 'Personalidad y habilidades blandas' },
...
export const MatchingPage: React.FC<Props> = ({ onNavigate }) => {
  const totalScore = Math.round(DIMENSIONES.reduce((acc, d) => acc + (d.score * d.pct) / 100, 0));
  const reached93  = totalScore >= 93;

  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-ink-900">Mi Match Score</h1>
        <p className="text-ink-500 text-sm mt-1">Así te evalúa el algoritmo de Worindev</p>
      </div>

      {/* Score principal */}
      <div className={`card-light p-8 text-center relative overflow-hidden ${reached93 ? 'border-l-4 border-accent-500' : ''}`}>
        <div className="relative inline-block mb-4">
          <svg viewBox="0 0 120 120" className="w-36 h-36 -rotate-90">
            <circle cx="60" cy="60" r="50" fill="none" stroke="#dde3ed" strokeWidth="8" />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-ink-500">puntuación</p>
          </div>
        </div>
        ...
        {reached93 && (
          <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent-100 text-accent-700 text-sm font-semibold border border-accent-200">
      ...
      {/* Dimensiones */}
      <div>
        <h2 className="text-lg font-semibold text-ink-900 mb-4">Desglose por dimensión</h2>
        <div className="space-y-4">
          {DIMENSIONES.map(d => (
            <div key={d.key} className="card-light p-5">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <div>
                      ...
                      <span className="text-ink-300 text-xs ml-2">({d.pct}% del total)</span>
                    </div>
                    <span className={`text-lg font-bold ${d.score >= 90 ? 'text-accent-600' : d.score >= 70 ? 'text-yellow-600' : 'text-ink-500'}`}>
                      {d.score}%
                    </span>
                  </div>
                  <p className="text-xs text-ink-500 mb-2">{d.desc}</p>
                  <div className="w-full bg-surface-border rounded-full h-2">
                    <div className={`h-2 rounded-full bg-gradient-to-r ${d.color} transition-all duration-700`} style={{ width: `${d.score}%` }} />
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Cómo mejorar */}
      <div className="card-light p-6 border-l-4 border-primary-500">
        <h3 className="font-semibold text-ink-900 mb-4 flex items-center gap-2">
          <TrendingUp size={18} className="text-primary-500" /> Cómo mejorar tu score
        </h3>
        <div className="space-y-3">
          {[
            ...
            <div key={a.action} className="flex items-center gap-3">
              <CheckCircle2 size={16} className={a.done ? 'text-accent-500' : 'text-ink-300'} />
              <span className={`text-sm flex-1 ${a.done ? 'text-ink-300 line-through' : 'text-ink-700'}`}>{a.action}</span>
              {!a.done && <span className="text-xs font-semibold text-accent-600">{a.impact}</span>}
            </div>
          ))}
        </div>
