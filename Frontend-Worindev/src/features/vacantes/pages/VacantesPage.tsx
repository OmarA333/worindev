...
import { Search, MapPin, Briefcase, ChevronRight, Plus } from 'lucide-react';

interface Props { onNavigate: (path: string) => void; }

const MOCK_VACANTES = [
  { id: '1', titulo: 'Desarrollador React Senior', empresa: 'TechCorp SAS',   ciudad: 'Medellín', modalidad: 'Híbrido',     salario: '$5M - $7M', match: 93, habilidades: ['React', 'TypeScript', 'Node.js'], tipoContrato: 'Indefinido', postulantes: 24 },
  { id: '2', titulo: 'Frontend Engineer',           empresa: 'Startup Digital', ciudad: 'Bogotá',   modalidad: 'Remoto',      salario: '$4M - $6M', match: 87, habilidades: ['Vue.js', 'CSS', 'Git'],           tipoContrato: 'Indefinido', postulantes: 18 },
  { id: '3', titulo: 'Analista de Datos',           empresa: 'DataCo',          ciudad: 'Cali',     modalidad: 'Presencial',  salario: '$3.5M - $5M', match: 72, habilidades: ['Python', 'SQL', 'Power BI'],   tipoContrato: 'Fijo',       postulantes: 31 },
  { id: '4', titulo: 'DevOps Engineer',             empresa: 'CloudSystems',    ciudad: 'Medellín', modalidad: 'Remoto',      salario: '$6M - $9M', match: 65, habilidades: ['AWS', 'Docker', 'Kubernetes'],   tipoContrato: 'Indefinido', postulantes: 12 },
  { id: '5', titulo: 'UX/UI Designer',              empresa: 'DesignHub',       ciudad: 'Bogotá',   modalidad: 'Híbrido',     salario: '$3M - $4.5M', match: 58, habilidades: ['Figma', 'Adobe XD'],          tipoContrato: 'Fijo',       postulantes: 45 },
];
...
export const VacantesPage: React.FC<Props> = ({ onNavigate }) => {
  ...
  const modalidadBadge = (m: string) =>
    m === 'Remoto'     ? 'bg-accent-100 text-accent-700'   :
    m === 'Híbrido'    ? 'bg-primary-100 text-primary-700' :
            ...
            <div key={v.id} onClick={() => setSelected(v)}
              ...
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-ink-900 text-sm truncate">{v.titulo}</h3>
                  <p className="text-ink-500 text-xs mt-0.5">{v.empresa}</p>
                </div>
                {!isEmpresa && (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="flex items-center gap-1 text-xs text-ink-500"><MapPin size={11} />{v.ciudad}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${modalidadBadge(v.modalidad)}`}>{v.modalidad}</span>
          ...
          {selected ? (
            <div className="card-light p-6 sticky top-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-xl font-bold text-ink-900">{selected.titulo}</h2>
                  <p className="text-ink-500 mt-1">{selected.empresa} · {selected.ciudad}</p>
                </div>
                {!isEmpresa && (
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                <span className={`text-xs px-3 py-1 rounded-full ${modalidadBadge(selected.modalidad)}`}>{selected.modalidad}</span>
                <span className="text-xs px-3 py-1 rounded-full bg-surface-bg text-ink-700 border border-surface-border">{selected.tipoContrato}</span>
                <span className="text-xs px-3 py-1 rounded-full bg-surface-bg text-ink-700 border border-surface-border">{selected.salario}</span>
                {isEmpresa && <span className="text-xs px-3 py-1 rounded-full bg-surface-bg text-ink-700 border border-surface-border">{selected.postulantes} postulantes</span>}
              </div>
              ...
              {!isEmpresa ? (
                ...
                <div className="flex gap-3">
                  ...
                  <button className="flex-1 py-2.5 rounded-xl font-semibold text-ink-700 bg-surface-bg border border-surface-border hover:border-ink-300 transition-all text-sm">
                    Editar vacante
                  </button>
                </div>
              )}
